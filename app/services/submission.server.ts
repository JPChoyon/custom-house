...
export async function createSubmission(shop: string, customerId: string, input: ManualDesignInput, client: ShopifyGraphqlClient) {
  ...
  if (!result.product) throw new DomainError("PRODUCT_NOT_FOUND", "Base product not found.", 404); if (result.product.origin?.value !== "global") throw new DomainError("NOT_GLOBAL", "Designs must start from a Global Product.", 422); if (result.product.mode?.value !== "customizable") throw new DomainError("NOT_CUSTOMIZABLE", "This product is not customizable.", 422);
  const idempotencyKey = submissionKey(shop, creator.id, normalized.baseProductId, normalized.savedDesignUrl);
  try { return await db.$transaction(async (tx) => { const submission = await tx.designSubmission.create({ data: { shop, creatorId: creator.id, ...normalized, idempotencyKey } }); await tx.auditLog.create({ data: { shop, actorType: "CUSTOMER", actorId: customerId, action: "submission.created", entityType: "DesignSubmission", entityId: submission.id } }); return submission; }); }
  catch (error) { if (error instanceof Error && error.message.includes("Unique constraint")) throw new DomainError("DUPLICATE_SUBMISSION", "This saved design was already submitted.", 409); throw error; }
}
